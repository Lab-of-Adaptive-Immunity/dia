---
title: "Hahtag analysis exp11 W2"
...
```{r parameters}
raw.matrix.path <- '../data/raw/GEX/exp11_W2_PE/raw_feature_bc_matrix/'
target.path <- 'Hashtags_11_W2/'
min.GE <- 200
hash.prop.lim <- 0.1
sample.strains <- c('H2', 'H3', 'H4', 'H5', 'H6', 'H7', 'H8', 'H9', 'H10')
strain.markers <- c('H2', 'H3', 'H4', 'H5', 'H6', 'H7', 'H8', 'H9', 'H10')
split.auto <- TRUE
cutoff.values <- c()
```
...
for(i  in 1:dim(marker.combin)[2]){
  ...
  color.def[(antibodies[marker.combin[1,i],] > cutoff.values[1]) & (antibodies[marker.combin[2,i],] > cutoff.values[2])] <- 'olivedrab'
  plot(antibodies[marker.combin[1,i],], antibodies[marker.combin[2,i],], main = paste0(marker.combin[1,i], ' vs. ', marker.combin[2,i], ' for experiment ', sep01), xlab = marker.combin[1,i], ylab = marker.combin[2,i], pch = 4, col = color.def, cex = 0.7)
...
In this case is a bit more complicated because there are two hashtags (1-6 as a first group, 7-10 as a second). However, there are some combinationbetween groups that do not appear as well (such as 1 with 7 or 8). Therefore, at first we remove cells that express more than one of hashtags 1-6, than more than one from groups 7-10, then those typical for those specific combinations.
