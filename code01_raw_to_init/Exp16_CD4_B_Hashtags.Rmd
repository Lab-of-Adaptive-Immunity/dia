...
```{r parameters}
raw.matrix.path <- '../data/raw/GEX/exp16_CD4_B/raw_feature_bc_matrix/'
target.path <- 'Hashtags_exp16_CD4_B/'
min.GE <- 200
hash.prop.lim <- 0.1
...
for(i  in 1:dim(marker.combin)[2]){
  ...
  color.def[(antibodies[marker.combin[1,i],] > cutoff.values[strain.markers == marker.combin[1,i]]) & (antibodies[marker.combin[2,i],] > cutoff.values[strain.markers == marker.combin[2,i]])] <- 'olivedrab'
...
print(paste('Doublets:', length(doublets)))
