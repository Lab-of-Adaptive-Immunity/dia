---
title: "Group 48 pipeline v00 m01"
...
```{r parameters}
raw.matrix.path <- '../data/raw/GEX/Exp08_GEX/raw_feature_bc_matrix/'
target.path <- 'Hashtags_08/'
min.GE <- 200
hash.prop.lim <- 0.1
sample.strains <- c('H1', 'H2', 'H4', 'H6', 'H7','H8', 'H9', 'H10')
strain.markers <- c('H1', 'H2', 'H4', 'H6', 'H7','H8', 'H9', 'H10')
split.auto <- TRUE
cutoff.values <- c()
```
...
#antibodies <- antibodies[,colnames(antibodies) %in% colnames(tcell)]
...
for(i in strain.markers){
   hist(antibodies[i,], col = 'purple', xlab = i, main = paste("Frequencies of ", i), breaks = max(antibodies[i,]))
}
