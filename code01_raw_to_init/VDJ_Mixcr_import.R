...
# - remove_suffix - whether remove number after asterisk returned by Mixcr for IMGT annotation
extract_gene_name_from_mixcr <- function(str, remove_suffix = T){
  if(is.na(str) | str == ''){
    return('')
  }
...
# - mixcr data are not used for doublet filtering, as they seem to be a bit unreliable for that
process.VDJ.GEX <- function(GEX.VDJ.dat.A, 
  ...
  GEX.VDJ.dat.A <- GEX.VDJ.dat.A %>% dplyr::arrange(barcode, umis) %>%
    group_by(barcode) %>% dplyr::filter(row_number() %in% c(n(), n()-1)) %>%
    dplyr::arrange(barcode, umis)
  
  GEX.VDJ.dat.B <- GEX.VDJ.dat.B %>% dplyr::arrange(barcode, umis) %>%
  ...
  if(!is.null(VDJ.dat)){
    ...
    int_vdj <- VDJ.dat %>% dplyr::filter(cdr3 != 'None' & (cdr3 != '')) %>%
      group_by(barcode, chain, cdr3) %>% dplyr::filter(row_number() == 1)
...
#
# builds up a vdj data table that contains 10X and Mixcr VDJs where we exclude OT-I VDJs but list cells that have them (alpha or beta)
# - path.GEX.VDJ.A, path.GEX.VDJ.B - path to Mixcr VDJ files for TCRA and TCRB respectively
# - path.10X.data - path to 10X VDJ data
# - dist.lim - number of errors allowed for matching, with 0.1 for substitution and 1 for other errors (default = 1 substitution)
build.VDJ.table.no.OT.I <- function(path.GEX.VDJ.A, path.GEX.VDJ.B, path.vdj.data = NULL, dist.lim = 0.1){
  GEX.VDJ.A <- read.csv(path.GEX.VDJ.A, sep = '\t')
  GEX.VDJ.B <- read.csv(path.GEX.VDJ.B, sep = '\t')
  vdj.data <- NULL
  if(!is.null(path.vdj.data)){
    vdj.data <- load.10X.VDJ(build.VDJ.table.no.OT.I)
  }
  GEX.VDJ.dat.A <- build.GEX.VDJ.table(GEX.VDJ.A, 'TRA', extend.barcode = TRUE, vdj.dat = vdj.data)
  GEX.VDJ.dat.B <- build.GEX.VDJ.table(GEX.VDJ.B, 'TRB', extend.barcode = TRUE, vdj.dat = vdj.data)
  
  # Get cell barcodes where either alpha or beta matches OT-I sequence. We allow 1 mismatch
  OT.I.alpha.dists <- stringdist::stringdist(GEX.VDJ.dat.A$cdr3, 'CAASDNYQLIW', weight = c(d = 1, i = 1, s = 0.1, t = 1))
  OT.I.beta.dists <- stringdist::stringdist(GEX.VDJ.dat.B$cdr3, 'CASSRANYEQYF', weight = c(d = 1, i = 1, s = 0.1, t = 1))
  
  OT.I.alpha <- GEX.VDJ.dat.A$barcode[OT.I.alpha.dists <= dist.lim] %>% unique
  OT.I.beta <- GEX.VDJ.dat.B$barcode[OT.I.beta.dists <= dist.lim] %>% unique
  
  # Remove those sequences
  GEX.VDJ.dat.A <- GEX.VDJ.dat.A %>% dplyr::filter(OT.I.alpha.dists >  dist.lim)
  GEX.VDJ.dat.B <- GEX.VDJ.dat.B %>% dplyr::filter(OT.I.beta.dists > dist.lim)  
  
  vdj.data <- process.VDJ.GEX(GEX.VDJ.dat.A, GEX.VDJ.dat.B, vdj.data)
  return.data <- list(vdj.data, OT.I.alpha, OT.I.beta)
  names(return.data) <- c('VDJ', 'OT.I.with.alpha', 'OT.I.with.beta')
  return(return.data)
}

# builds up a table for a single chain from GEX only
# useful when you want to mass analyze vdj for single chain only
# - path.GEX.VDJ - path to csv of Mixcr VDJ table
# - chain - chain to specify
build.VDJ.t.singular <- function(path.GEX.VDJ, chain = 'TRB'){
  GEX.VDJ <- read.csv(path.GEX.VDJ, sep = '\t')
  GEX.VDJ.dat <- build.GEX.VDJ.table(GEX.VDJ, chain, extend.barcode = TRUE, vdj.dat = NULL)
  return(GEX.VDJ.dat)
}
