...
create.VDJ.metadata <- function(seurat.obj, 
  ...
  input.vdj <- input.vdj %>% dplyr::filter(grepl('[T|t]rue', productive)) %>% 
    dplyr::arrange(chain, vdj_source, barcode, desc(reads), desc(umis))
...
# finds and returns list of cells the VDJs of which have same sequence as OT-I cells
...
filter.out.OT.I <- function(vdj.data, dist.lim = 0.1){
  VDJ.dat.A <- vdj.data[vdj.data$chain == 'TRA',]
  VDJ.dat.B <- vdj.data[vdj.data$chain == 'TRB',]  
  VDJ.dat.rem <- vdj.data[vdj.data$chain %nin% c('TRA', 'TRB'),]
  
  # Get cell barcodes where either alpha or beta matches OT-I sequence. We allow 1 mismatch
  OT.I.alpha.dists <- stringdist::stringdist(VDJ.dat.A$cdr3, 'CAASDNYQLIW', weight = c(d = 1, i = 1, s = 0.1, t = 1))
  OT.I.beta.dists <- stringdist::stringdist(VDJ.dat.B$cdr3, 'CASSRANYEQYF', weight = c(d = 1, i = 1, s = 0.1, t = 1))
  
  OT.I.alpha <- VDJ.dat.A$barcode[OT.I.alpha.dists <= dist.lim] %>% unique
  OT.I.beta <- VDJ.dat.B$barcode[OT.I.beta.dists <= dist.lim] %>% unique
  
  # Remove those sequences
  VDJ.dat.A <- VDJ.dat.A %>% dplyr::filter(OT.I.alpha.dists >  dist.lim)
  VDJ.dat.B <- VDJ.dat.B %>% dplyr::filter(OT.I.beta.dists > dist.lim)  
  
  vdj.data.filt <- rbind(VDJ.dat.A, VDJ.dat.B, VDJ.dat.rem)
  return.data <- list(vdj.data.filt, OT.I.alpha, OT.I.beta)
